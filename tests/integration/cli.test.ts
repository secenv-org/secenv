...
describe('CLI Integration', () => {
  ...
  it('should initialize successfully', async () => {
    ...
    expect(fs.existsSync(path.join(testDir, '.env.enc'))).toBe(true);
    expect(fs.readFileSync(path.join(testDir, '.gitignore'), 'utf-8')).toContain('.env.enc');
  });
  ...
  it('should set and get values', async () => {
    ...
    const envContent = fs.readFileSync(path.join(testDir, '.env.enc'), 'utf-8');
