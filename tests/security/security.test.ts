...
describe('Security Tests', () => {
  ...
  it('should use different ciphertexts for same plaintext (nonce randomness)', async () => {
    ...
    expect(await decrypt(identity, enc1)).toBe(plaintext);
    expect(await decrypt(identity, enc2)).toBe(plaintext);
  });
  ...
  it('should include .env.enc in .gitignore during init', async () => {
    await runCLI(['init']);
    const gitignore = fs.readFileSync(path.join(testDir, '.gitignore'), 'utf-8');
    expect(gitignore).toContain('.env.enc');
  });
